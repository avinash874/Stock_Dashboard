...
async function selectCompany(symbol) {
  ...
  if (pred && pred.fitted_close && pred.fitted_close.length === closes.length) {
    datasets.push({
      label: "ML linear fit",
      data: pred.fitted_close,
      borderColor: "#c9a227",
      borderDash: [4, 3],
      tension: 0.1,
      pointRadius: 0,
  ...
  const pad5 = (arr) => [...arr, ...Array(5).fill(null)];

  if (pred && pred.predicted_next_5_closes && pred.predicted_next_5_closes.length) {
    const hist = series.data;
    const lastDate = new Date(hist[hist.length - 1].date);
    const extLabels = [...labels];
    const lastClose = closes[closes.length - 1];
    const bridge = [...Array(closes.length - 1).fill(null), lastClose];
    for (let i = 1; i <= 5; i++) {
      const d = new Date(lastDate);
      d.setDate(d.getDate() + i);
      extLabels.push(d.toISOString().slice(0, 10));
      bridge.push(pred.predicted_next_5_closes[i - 1]);
    }
    datasets[0].data = pad5(datasets[0].data);
    if (datasets[1] && datasets[1].label === "7d MA") datasets[1].data = pad5(datasets[1].data);
    const fitIdx = datasets.findIndex((d) => d.label === "ML linear fit");
    if (fitIdx >= 0) datasets[fitIdx].data = pad5(datasets[fitIdx].data);
    datasets.push({
      label: "ML extrapolation (5d)",
      data: bridge,
      borderColor: "#f0c14d",
      borderDash: [2, 4],
      pointRadius: 2,
